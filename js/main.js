...
class HealthManagementSystem {
    constructor() {
        ...
        this.teachers = [];
        this.charts = {};
        this.dateFilter = 'today'; // デフォルトは「今日」
        this.currentSort = 'sortDateDesc'; // デフォルトは日時降順
    ...
    setupEventListeners() {
        ...
        if (navHealthCheck) navHealthCheck.addEventListener('click', () => this.showView('healthCheck'));
        if (navStudents) navStudents.addEventListener('click', () => this.showView('students'));
        if (navRecords) navRecords.addEventListener('click', () => this.showView('records'));
        if (navConsultations) navConsultations.addEventListener('click', () => this.showView('consultations'));
        if (navTeachers) navTeachers.addEventListener('click', () => this.showView('teachers'));
        ...
        if (filterToday) filterToday.addEventListener('click', () => this.setDateFilter('today'));
        if (filterWeek) filterWeek.addEventListener('click', () => this.setDateFilter('week'));
        if (filterMonth) filterMonth.addEventListener('click', () => this.setDateFilter('month'));
        if (filterAll) filterAll.addEventListener('click', () => this.setDateFilter('all'));
        ...
        if (healthForm) healthForm.addEventListener('submit', (e) => this.handleHealthSubmit(e));
        if (clearForm) clearForm.addEventListener('click', () => this.clearHealthForm());
        ...
        const importStudentsCSVBtn = document.getElementById('importStudentsCSVBtn');
        ...
        if (exportStudentsCSVBtn) exportStudentsCSVBtn.addEventListener('click', () => this.exportStudentsCSV());
        if (importStudentsCSVBtn) importStudentsCSVBtn.addEventListener('click', () => this.importStudentsCSV());
        if (deleteAllStudentsBtn) deleteAllStudentsBtn.addEventListener('click', () => this.deleteAllStudents());
        if (studentForm) studentForm.addEventListener('submit', (e) => this.handleStudentSubmit(e));
        if (cancelStudent) cancelStudent.addEventListener('click', () => this.hideStudentModal());
        
        // 児童検索・フィルター
        const searchStudentInput = document.getElementById('searchStudentInput');
        const filterStudentGrade = document.getElementById('filterStudentGrade');
        const filterStudentClass = document.getElementById('filterStudentClass');
        const filterStudentStatus = document.getElementById('filterStudentStatus');
        const clearStudentFilter = document.getElementById('clearStudentFilter');
        
        if (searchStudentInput) searchStudentInput.addEventListener('input', () => this.filterStudentsTable());
        if (filterStudentGrade) filterStudentGrade.addEventListener('change', () => this.filterStudentsTable());
        if (filterStudentClass) filterStudentClass.addEventListener('input', () => this.filterStudentsTable());
        if (filterStudentStatus) filterStudentStatus.addEventListener('change', () => this.filterStudentsTable());
        if (clearStudentFilter) clearStudentFilter.addEventListener('click', () => this.clearStudentFilter());

        // 記録フィルター
        const filterRecords = document.getElementById('filterRecords');
        const filterGrade = document.getElementById('filterGrade');
        const filterHealthStatus = document.getElementById('filterHealthStatus');
        
        if (filterRecords) filterRecords.addEventListener('click', () => this.filterRecords());
        if (filterHealthStatus) filterHealthStatus.addEventListener('change', () => this.filterRecords());
        if (filterGrade) {
        ...
        // 記録ソート
        const sortButtons = [
            { id: 'sortDateDesc', handler: () => this.setSortOrder('sortDateDesc') },
            { id: 'sortDateAsc', handler: () => this.setSortOrder('sortDateAsc') },
            { id: 'sortTempDesc', handler: () => this.setSortOrder('sortTempDesc') },
            { id: 'sortTempAsc', handler: () => this.setSortOrder('sortTempAsc') },
            { id: 'sortStressDesc', handler: () => this.setSortOrder('sortStressDesc') },
            { id: 'sortStressAsc', handler: () => this.setSortOrder('sortStressAsc') },
            { id: 'sortName', handler: () => this.setSortOrder('sortName') },
            { id: 'sortGrade', handler: () => this.setSortOrder('sortGrade') }
        ];
        
        sortButtons.forEach(({ id, handler }) => {
            const btn = document.getElementById(id);
            if (btn) btn.addEventListener('click', handler);
        });

        // 相談管理
        const filterConsultations = document.getElementById('filterConsultations');
        ...
        if (filterConsultations) filterConsultations.addEventListener('click', () => this.filterConsultations());
        if (refreshConsultationsBtn) refreshConsultationsBtn.addEventListener('click', () => this.loadConsultationsData());
        if (exportToSheetsBtn) exportToSheetsBtn.addEventListener('click', () => this.exportToGoogleSheets());
        if (cancelConsultation) cancelConsultation.addEventListener('click', () => this.hideConsultationModal());
        if (saveConsultation) saveConsultation.addEventListener('click', () => this.saveConsultationResponse());

        // 静的データ管理
        const exportDataBtn = document.getElementById('exportDataBtn');
        const importDataBtn = document.getElementById('importDataBtn');
        
        if (exportDataBtn) exportDataBtn.addEventListener('click', () => this.exportStaticData());
        if (importDataBtn) importDataBtn.addEventListener('click', () => this.importStaticData());

        // 教員管理
        const addTeacherBtn = document.getElementById('addTeacherBtn');
        const importTeachersCSVBtn = document.getElementById('importTeachersCSVBtn');
        ...
        const teacherNameSearch = document.getElementById('teacherNameSearch');
        ...
        if (addTeacherBtn) addTeacherBtn.addEventListener('click', () => this.showAddTeacherModal());
        if (importTeachersCSVBtn) importTeachersCSVBtn.addEventListener('click', () => this.importTeachersCSV());
        if (exportTeachersBtn) exportTeachersBtn.addEventListener('click', () => this.exportTeachersToCSV());
        if (refreshTeachersBtn) refreshTeachersBtn.addEventListener('click', () => this.loadTeachersData());
        if (searchTeachers) searchTeachers.addEventListener('click', () => this.searchTeachers());
        if (teacherNameSearch) {
            teacherNameSearch.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') this.searchTeachers();
            });
        }
        if (cancelTeacher) cancelTeacher.addEventListener('click', () => this.hideTeacherModal());
        if (teacherForm) teacherForm.addEventListener('submit', (e) => this.handleTeacherSubmit(e));
        if (cancelDeleteTeacher) cancelDeleteTeacher.addEventListener('click', () => this.hideDeleteTeacherModal());
        if (confirmDeleteTeacher) confirmDeleteTeacher.addEventListener('click', () => this.confirmDeleteTeacher());
        ...
        if (deleteTeacherModal) {
            deleteTeacherModal.addEventListener('click', (e) => {
                if (e.target === e.currentTarget) {
    ...
    async loadData() {
        try {
            ...
            if (recordsResponse.ok) {
                ...
                // symptomsが文字列の場合は配列に変換
                this.healthRecords = (recordsData.data || []).map(record => {
        ...
        ['filterToday', 'filterWeek', 'filterMonth', 'filterAll'].forEach(id => {
            ...
            if (btn) {
                if (id === 'filter' + filter.charAt(0).toUpperCase() + filter.slice(1)) {
                    btn.className = 'px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium';
                } else {
                    btn.className = 'px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors font-medium';
                }
            }
        });
    ...
    updateDateRangeLabel() {
        ...
        const label = document.getElementById('filterDateRange');
        if (label) {
            ...
            label.textContent = startStr === endStr ? startStr : `${startStr} 〜 ${endStr}`;
        }
    }
    ...
    getDateRange() {
        ...
        switch (this.dateFilter) {
            ...
            case 'week':
                ...
                start.setDate(today.getDate() - today.getDay()); // 今週の日曜日
                end = new Date(start);
                end.setDate(start.getDate() + 6); // 今週の土曜日
                break;
            case 'month':
    ...
    filterRecords() {
        ...
        const healthStatus = document.getElementById('filterHealthStatus')?.value || '';
        ...
        // 現在のソート設定を適用
        this.applySorting(filteredRecords);
    ...
    getHealthStatus(record) {
        ...
        const stressLevel = parseInt(record.stress_level) || 0;
        
        if (temp >= 37.5 || symptoms.includes('発熱')) {
            return 'fever';
        } else if (temp >= 37.0 || symptoms.length > 0 || stressLevel >= 4) {
            return 'caution';
        } else {
            return 'healthy';
        }
    ...
    setSortOrder(sortType) {
        ...
        // ソートボタンのスタイル更新
        const sortButtons = [
            'sortDateDesc', 'sortDateAsc', 'sortTempDesc', 'sortTempAsc',
            'sortStressDesc', 'sortStressAsc', 'sortName', 'sortGrade'
        ];
        ...
        sortButtons.forEach(btnId => {
            ...
            if (btn) {
                if (btnId === sortType) {
                    btn.className = 'px-3 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition-colors';
                } else {
                    btn.className = 'px-3 py-1.5 bg-gray-200 text-gray-700 text-sm rounded hover:bg-gray-300 transition-colors';
                }
            }
        });
    ...
    applySorting(records) {
        const sortType = this.currentSort || 'sortDateDesc';
        ...
        switch (sortType) {
            ...
            case 'sortDateAsc':
                ...
                break;
            case 'sortTempDesc':
                records.sort((a, b) => (parseFloat(b.temperature) || 0) - (parseFloat(a.temperature) || 0));
                break;
            case 'sortTempAsc':
                records.sort((a, b) => (parseFloat(a.temperature) || 0) - (parseFloat(b.temperature) || 0));
                break;
            case 'sortStressDesc':
                records.sort((a, b) => (parseInt(b.stress_level) || 0) - (parseInt(a.stress_level) || 0));
                break;
            case 'sortStressAsc':
                records.sort((a, b) => (parseInt(a.stress_level) || 0) - (parseInt(b.stress_level) || 0));
                break;
            case 'sortName':
                records.sort((a, b) => {
                    const studentA = this.students.find(s => s.id === a.student_id);
                    const studentB = this.students.find(s => s.id === b.student_id);
            ...
            case 'sortGrade':
                records.sort((a, b) => {
                    ...
                    return studentA.class.localeCompare(studentB.class, 'ja');
                });
                break;
        }
    }
    ...
    displayRecords(records) {
        ...
        records.forEach(record => {
                ...
                // symptomsの表示処理（文字列・配列両対応）
                let symptoms = '-';
                if (record.symptoms) {
                    if (Array.isArray(record.symptoms)) {
                        symptoms = record.symptoms.length > 0 ? record.symptoms.join(', ') : '-';
                ...
                const stressLevel = record.stress_level || '-';
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        ${symptoms !== '-' ? symptoms.split(', ').map(symptom => 
                            `<span class="symptom-mild">${symptom}</span>`
                        ).join(' ') : '-'}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${mood}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${stressLevel}</td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.healthSystem = app; // グローバルアクセス用
});
